...
use crate::common::types::{ErssProviderConfig, SosBroadcasterConfig, SosTraceProvider};
use crate::tools::error::AppError;

/// Trait for external location providers that receive SOS location pings.
///
...
///   1. Add a variant to `SosTraceProvider` in common/types.rs with its own config struct.
...
static ERSS_CLIENT: Lazy<Client> = Lazy::new(|| {
    Client::builder()
...
impl ExternalLocationProvider for ErssLocationProvider {
    async fn send_ping(&self, lat: f64, lon: f64, datetime_str: &str) -> Result<(), AppError> {
        ...
        let url = Url::parse(&url_str)
            .map_err(|e| AppError::InvalidRequest(format!("Invalid SOS trace URL: {}", e)))?;
            ...
            Box::new(|resp| {
                Box::pin(async move {
                    if resp.status() == StatusCode::UNAUTHORIZED {
                        AppError::ErssTokenExpired
                    } else {
                        AppError::ExternalAPICallError(format!("SOS trace HTTP {}", resp.status()))
                    }
                })
            }),
        )
