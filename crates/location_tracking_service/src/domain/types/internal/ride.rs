...
/// For SOS `EntityStart`, include `broadcaster_config` to atomically register the broadcaster
...
/// Request body for registering a broadcaster config. Sent as part of `EntityStart` for SOS
/// entities. The `provider` field carries provider-kind discriminant + provider-specific fields.
/// Named generically so future entity types (e.g. rides) can reuse the same shape.
...
/// Response from an external SOS provider reauth endpoint.
#[derive(Serialize, Deserialize, Debug)]
#[serde(rename_all = "camelCase")]
pub struct ExternalSosReauthResponse {
    pub access_token: String,
    pub expires_at: i64,
}
