...
pub fn add_to_server (conn: &mut redis::Connection, list: Vec<(f64, f64, String)>) {
  let mut hash_list: Vec<(String, &String)> = list.iter()
  .map(|x| 
    (to_hash(x.0, x.1), &x.2))
  .collect();
  ...
  let new_list: Vec<(f64, f64, &String)> = hash_list.iter()
  .map(|x| {
...
pub fn push_coord (list: &mut Vec<(f64, f64, String)>, item: (f64, f64, &str)) -> Vec<(f64, f64, String)> {
